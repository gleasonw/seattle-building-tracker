import { pgTable, varchar, text, integer, decimal, date, boolean, timestamp, index } from 'drizzle-orm/pg-core';
...
export const buildingPermits = pgTable('building_permits', {
  ...
  initialReviewCompleteDate: date('initial_review_complete_date', { mode: 'string' }),
  planReviewCompleteDate: date('plan_review_complete_date', { mode: 'string' }),
  ...
  localUpdatedAt: timestamp('local_updated_at', { mode: 'string' }).defaultNow(),
  ...
  permitClassMappedIdx: index('permit_class_mapped_idx').on(table.permitClassMapped),
  locationIdx: index('location_idx').on(table.originalCity, table.originalZip),
  relatedMupIdx: index('related_mup_idx').on(table.relatedMup),
  parentPermitNumIdx: index('parent_permit_num_idx').on(table.parentPermitNum),
  remoteUpdatedAtIdx: index('remote_updated_at_idx').on(table.remoteUpdatedAt),
}));
