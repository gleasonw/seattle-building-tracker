...
export default function ConstructionClient({
  ...
  const createSeattleDataUrl = () => {
    ...
    if (initialParams.tableStart || initialParams.start) {
      conditions.push(
        `\`${dateField}\` >= '${
          initialParams.tableStart || initialParams.start
        }T00:00:00'`
      );
    }
    if (initialParams.tableEnd || initialParams.end) {
      conditions.push(
        `\`${dateField}\` <= '${
          initialParams.tableEnd || initialParams.end
        }T23:59:59'`
      );
    }
